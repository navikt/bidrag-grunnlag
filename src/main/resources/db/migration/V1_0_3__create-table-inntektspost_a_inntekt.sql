-- Table: inntekt_ainntektspost

-- DROP TABLE inntekt_ainntektspost;

CREATE TABLE IF NOT EXISTS inntekt_ainntektspost
(
    inntektspost_id integer NOT NULL GENERATED BY DEFAULT AS IDENTITY ( INCREMENT 1 START 1 MINVALUE 1 MAXVALUE 2147483647 CACHE 1 ),
    inntekt_id integer NOT NULL,
    utbetalingsperiode char(7) NOT NULL,
    opptjeningsperiode_fra date,
    opptjeningsperiode_til date,
    opplysningspliktig_id varchar(255),
    type varchar(255) NOT NULL,
    fordel_type varchar(255) NOT NULL,
    beskrivelse varchar(255) NOT NULL,
    belop float NOT NULL,
    CONSTRAINT inntekt_ainntektspost_pkey PRIMARY KEY (inntektspost_id),
    CONSTRAINT inntekt_ainntekt_fkey FOREIGN KEY (inntekt_id)
        REFERENCES inntekt_ainntekt (inntekt_id) MATCH SIMPLE
        ON UPDATE NO ACTION
        ON DELETE NO ACTION,
    UNIQUE (inntekt_id, utbetalingsperiode, opplysningspliktig_id, type, fordel_type, beskrivelse)
)

    TABLESPACE pg_default;
